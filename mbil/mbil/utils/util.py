...
from mbil.utils import init_logger
...
def setup(args, ask_prompt=True):
    ...
    - config will store a json parse of the arg configs used for experiment. Could be used to load back in
...
def get_dynamics_id(args):
    ...
    #num_traj = args.num_trajs
    num_traj = 0
    return f"{env}-seed={seed}-num_experts={num_traj}-num_models={n_models}"
...
def get_experiment_id(args):
    ...
    pg_update = args.pg_iter
    n_minmax = args.n_minmax
    w_update = args.update_type
    cost_lr = args.cost_lr
    return f"{env}-seed={seed}-lambda={lambda_b}-pg_iter={pg_update}-n_minmax={n_minmax}-id={experiment_id}"
