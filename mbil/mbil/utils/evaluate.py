...
def evaluate(n_iter, logger, writer, args, env, policy, reward_func, num_traj=10):
    ...
    # Compute MMD (S) or (S, S'
    greedy_x = np.concatenate([traj['observations'] for traj in greedy_samples], axis=0)
    sample_x = np.concatenate([traj['observations'] for traj in samples], axis=0)

    if args.use_next_state:
        greedy_next_x = np.concatenate([traj['next_observations'] for traj in greedy_samples], axis=0)
        sample_next_x = np.concatenate([traj['next_observations'] for traj in samples], axis=0)
        greedy_x = np.concatenate([greedy_x, greedy_next_x], axis=1)
        sample_x = np.concatenate([sample_x, sample_next_x], axis=1)
