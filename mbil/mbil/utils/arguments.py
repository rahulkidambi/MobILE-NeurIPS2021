...
def get_args():
    ...
    parser.add_argument('--n_models', type=int,
                        help='Number of dynamics models in ensemble', default=2)
    parser.add_argument('--n_epochs', type=int,
                        help='Number of epochs to train models', default=50)
    parser.add_argument('--grad_clip', type=float,
                        help='Max Gradient Norm', default=1.0)
    parser.add_argument('--dynamics_lr', type=float,
                        help='LR for dynamics training', default=5e-3)
    parser.add_argument('--dynamics_momentum', type=float,
                        help='Momentum for dynamics training', default=0.99)
    parser.add_argument('--dynamics_optim', type=str,
                        help='Optimizer to use [sgd, adam]', default='sgd')
    parser.add_argument('--dynamics_model_hidden', type=int,
                        nargs='+', help='hidden dims for dynamics_model', default=[1024, 1024])
    parser.add_argument('--n_dynamics_samples', type=int,
                        help='Number of samples per dynamics model step', default=4000)
    parser.add_argument('--buffer_size', type=int,
                        help='Size of buffer for dynamics training', default=40000)

    # Dynamics Model Pretraining Arguments
    parser.add_argument('--n_pretrain_epochs', type=int,
                        help='Number of epochs to pretrain models', default=100)
    parser.add_argument('--n_pretrain_samples', type=int,
                        help='Number of samples for pretrain dynamics', default=4000)
    parser.add_argument('--greedy_pretrain', action='store_true',
                        help='turns on greedy sampling for pretrain samples', default=False)
    ...
    parser.add_argument('--update_bw', action='store_true',
                        help='turns on updating bandwidth', default=False)
    parser.add_argument('--use_next_state', action='store_true', help="do (s, s') instead of (s)", default=False)
    ...
    parser.add_argument('--cg_iter', type=int,
                        help='Number of CG iterations', default=25)
    ...
    parser.add_argument('--n_iter', type=int, help='Number of MOBILE iterations to run', default=300)
    parser.add_argument('--n_minmax', type=int, help='Number of minmax IL iterations to run per dynamics training', default=10)
    parser.add_argument('--pg_iter', type=int, help='Number of pg steps per MMD update', default=1)
