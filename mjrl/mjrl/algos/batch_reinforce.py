...
try:
    from mbil.sampler import mb_sampler
except:
    print("MBIL not installed")
...
class BatchREINFORCE:
    ...
    def train_step(self, N,
        ...
        if reward_kwargs is not None:
            # NOTE: Reward is only updated on the first
            if reward_kwargs['i'] == 0:
                # ======== FIT REWARD WITH SAMPLES COLLECTED FOR TRPO ========
                # NOTE: this is (S) specific
                cost_input = np.concatenate([traj['observations'] for traj in paths], axis=0)
                if reward_kwargs['use_next_state']:
                    next_state = np.concatenate([traj['next_observations'] for traj in paths], axis=0)
                    cost_input = np.concatenate([cost_input, next_state], axis=1)
                if reward_kwargs['update_bw']:
                    old_bw = reward_kwargs['reward_func'].bw
                    #pdb.set_trace()
                    reward_kwargs['reward_func'].update_bandwidth(cost_input)
                    reward_kwargs['logger'].info(f">>>>> Updated Bandwidth from {old_bw} to {reward_kwargs['reward_func'].bw}")
            ...
            for traj in paths:
                ...
                if not reward_kwargs['use_ground_truth']:
